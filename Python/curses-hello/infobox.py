...
class Infobox:
    ...
    def on_key_event(self, key) -> bool:
        ...
        if key == ord('q'):
            ...
            self.textpad_pad_y = max(
                0,
                self.textpad_pad_y - 1
            )
            self._refresh()
            return True
            ...
            self.textpad_pad_y = min(
                self.textpad_pad_y + 1,
                self.n_lines - self.textpad_rows - 2
            )
            self._refresh()
            return True
        elif key == ord('h') or key == curses.KEY_LEFT:
            self.textpad_pad_x = max(
                0,
                self.textpad_pad_x - 1
            )
            self._refresh()
            return True
            ...
            self.textpad_pad_x = min(
                self.textpad_pad_x + 1,
                self.n_columns - self.textpad_cols - 2
            )
            self._refresh()
            return True
        return False
