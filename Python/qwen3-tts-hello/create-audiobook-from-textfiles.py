...
./text/, override with --text-dir), cleans conversion leftovers,
splits each chapter at sentence boundaries into chunks of at most
--max-chars characters, synthesizes every chunk with Qwen3-TTS, and
stitches the chunk audio into one WAV file per chapter.
...
class Voice(NamedTuple):
    """Speaker, language, and optional style instruction for synthesis."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Qwen3-TTS and write one stitched WAV file per chapter."
        ),
    )
    parser.add_argument(
        "--text-dir",
        type=Path,
        default=Path("text"),
        help="Directory containing chapter_NN.txt input files (default: text)",
    )
    parser.add_argument(
        "--audio-dir",
        type=Path,
    ...
    parser.add_argument(
        "--speaker",
        default="Ryan",
        help="Speaker voice name supported by the CustomVoice model (default: Ryan)",
    )
    parser.add_argument(
        "--language",
        default="English",
        help="Language of the texts (default: English)",
    )
    parser.add_argument(
        "--instruct",
        default=None,
        help=(
            "Optional voice instruction controlling emotion and style, "
            "for example 'Read like an audiobook narrator'. Default: none"
        ),
    )
    parser.add_argument(
        "--max-chars",
        ...
        default=300,
        help=(
            "Maximum characters per synthesis call. The TTS model degrades "
            "on long inputs (voice drift, repeated words), so each chapter "
            "is split at sentence boundaries into chunks up to this size - "
            "roughly 2-4 sentences or 10-20 seconds of audio per chunk. "
            "Lower values give a more stable voice but slower runtime and "
            "more stitch points; higher values reduce seams but risk "
            "mid-chunk artifacts. Default: 300"
        ),
    )
    parser.add_argument(
        "--gap",
...
def read_paragraphs(path: Path) -> list[str]:
    """Join hard-wrapped lines into paragraphs and drop page-number lines."""
...
def discover_chapters(
    ...
    if not found:
        message = f"No chapter_*.txt files found in {text_dir}."
        raise SystemExit(message)
    if only is None:
...
def main() -> None:
    ...
    print(f"Processing {len(chapters)} chapter(s) with speaker '{args.speaker}'.")
