...
void draw(){
  ...
  for(int i=0;i<num;i++){
    // draw the circle
    strokeWeight( rad[i]);
    stroke(  noise((loc[i].x+loc[i].y+millis())*0.001)*255,
             noise((loc[i].x)*0.005)*255,
             noise((loc[i].y)*0.005)*255,
             512-rad[i]);
    
    point( width/2+loc[i].x,height/2+loc[i].y);
...
void getCircle(int i){
  ...
  while(attempt<1000&&solution==false){
    attempt++;
    // make a circle so small it isnt visible
    float newRad = random(0,0.02);
