/* OpenProcessing Tweak of *@*http://www.openprocessing.org/sketch/153392*@* */
...
class Ball {
  ...
  void display() {
    ...
    if (col_show) {
      ...
      if (colliding && col_test) fill(img.get((int)x,(int)y), 255);
      else fill(img.get((int)x,(int)y), 100);
    }
